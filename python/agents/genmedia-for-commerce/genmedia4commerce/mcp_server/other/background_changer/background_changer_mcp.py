...
async def run_background_changer(
    ...
    if not background_description and not background_image_base64:
        return {"error": "Either background_description or background_image_base64 is required."}
    ...
    if reference_face is None:
        return {"error": "No face detected in the person image. Please upload a clearer image."}
    ...
    async def generate_variation(idx):
        ...
        step1_eval = await loop.run_in_executor(
            ...
            nano_client, step1_image, reference_face
        )

        if step2_image is not None:
            step2_eval = await loop.run_in_executor(
                None, evaluate_background_change_image,
                nano_client, step2_image, reference_face
            )
