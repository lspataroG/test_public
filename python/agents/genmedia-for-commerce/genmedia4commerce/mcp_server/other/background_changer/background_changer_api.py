...
async def change_background_endpoint(
    ...
    try:
        ...
        background_image_bytes = await background_image.read() if background_image else None
            ...
            async def generate_variation(idx):
                try:
                    ...
                    if gen_result is None:
                        await result_queue.put({"index": idx, "image": None, "error": "Generation failed"})
                        return
                    ...
                    step1_eval = await loop.run_in_executor(
                        None, evaluate_background_change_image,
                        nano_client, step1_image, reference_face_processed
                    )

                    if step2_image is not None:
                        step2_eval = await loop.run_in_executor(
                            None, evaluate_background_change_image,
                            nano_client, step2_image, reference_face_processed
                        )
                        if step2_eval["similarity_percentage"] >= step1_eval["similarity_percentage"]:
                    ...
                    await result_queue.put({
                        "index": idx, "image": best_image, "evaluation": best_eval,
                    })
                except Exception as e:
