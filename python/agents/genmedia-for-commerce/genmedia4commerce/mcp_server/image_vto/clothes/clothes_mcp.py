...
async def run_image_vto_clothes(
    ...
    async for result in run_image_vto(
        ...
        if result.status == "ready" and result.image is not None:
            ...
            logger.info(f"[MCP image_vto_clothes] Variation {getattr(result, 'index', '?')} ready, score={score:.2f}")
            if score > best_score:
                best_score = score
                best_result = result
    ...
    response = {
        "image_base64": best_result.image_base64 or base64.b64encode(best_result.image).decode("utf-8"),
