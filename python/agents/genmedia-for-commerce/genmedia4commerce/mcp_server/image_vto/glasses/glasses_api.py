...
_glasses_workflow_dir = os.path.join(
    ...
    os.pardir, os.pardir,  # mcp_server/
    "workflows", "image_vto", "glasses"
)
    ...
    if os.path.exists(_images_dir):
        for filename in os.listdir(_images_dir):
            if filename.lower().endswith((".png", ".jpg", ".jpeg")):
                image_files.append({"url": f"/glasses/images/{filename}", "name": filename})
...
async def generate_vto_endpoint(
    ...
    if not glasses_images:
        raise HTTPException(status_code=400, detail="At least one glasses image is required")
