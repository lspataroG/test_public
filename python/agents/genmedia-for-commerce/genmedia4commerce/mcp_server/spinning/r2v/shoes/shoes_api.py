...
_shoes_workflow_dir = os.path.normpath(
    os.path.join(_script_dir, os.pardir, os.pardir, os.pardir, os.pardir,
                 "workflows", "spinning", "r2v", "shoes")
)
            ...
            if os.path.isdir(folder_path) and folder_name.startswith("product_"):
                ...
                for filename in sorted(os.listdir(folder_path)):
                    if filename.lower().endswith((".png", ".jpg", ".jpeg")):
                        image_url = f"/shoes/spinning/images/products/{folder_name}/{filename}"
...
async def generate_prompt_with_position_r2v(all_images: list[UploadFile] = File(...)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to generate R2V prompt: {e}")
...
async def generate_video_r2v(
    ...
    try:
        ...
        logger.error(f"Error generating R2V video for index {index}: {e}", exc_info=True)
...
async def run_pipeline_endpoint_r2v(payload: dict = Body(...)):
    ...
    product_consistency_model = payload.get("product_consistency_model", "gemini-3-flash-preview")
    product_id = payload.get("product_id")
    gcs_bucket = payload.get("gcs_bucket")
    gcs_destination_prefix = payload.get("gcs_destination_prefix", "shoe_spinning_outputs")
    ...
    try:
        ...
        for idx, img_b64 in enumerate(images_base64):
            try:
                ...
                raise HTTPException(
                    status_code=400, detail=f"Invalid base64 encoding for image {idx}: {str(e)}"
                )
