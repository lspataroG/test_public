...
from mcp_server.product_enrichment.product_fitting.product_fitting_mcp import run_product_fitting
from mcp_server.image_vto.image_vto_mcp import run_image_vto
from mcp_server.video_vto.video_vto_mcp import run_video_vto
from mcp_server.other.background_changer.background_changer_mcp import run_background_changer
...
async def product_fitting(
    ...
    logger.info(f"[product_fitting] Called with {len(garment_images_base64)} images, ethnicity={ethnicity}")
    try:
...
async def image_vto(
    ...
    """
    logger.info(f"[image_vto] Called with {len(product_images_base64)} images, is_glasses={is_glasses}")
    try:
...
async def video_vto(
    ...
    """
    logger.info(f"[video_vto] Called with {len(product_images_base64)} images, is_glasses={is_glasses}")
    try:
...
async def product_spinning(
    ...
    """
    logger.info(f"[product_spinning] Called with {len(images_base64)} images, is_shoes={is_shoes}")
    try:
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting MCP server with transport={args.transport}, port={args.port}")
