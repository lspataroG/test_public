...
async def run_video_vto(
    ...
    """
    if is_glasses:
        logger.info(f"[video_vto] Routing to glasses pipeline ({len(product_images_base64)} images)")
        ...
        logger.info(f"[video_vto] Routing to clothes pipeline ({len(product_images_base64)} garments)")
