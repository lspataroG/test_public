...
from workflows.video_vto.glasses.pipeline import post_process_video, run_generation_pipeline
...
async def run_glasses_video_generate(
    ...
    logger.info(f"[MCP glasses_video_generate] Complete. {len(result.get('videos', []))} videos")
    return result
...
async def run_glasses_video_regenerate(
    ...
    """
    from workflows.video_vto.glasses.pipeline import RegenerationRequest, run_regeneration_pipeline
    ...
    logger.info(f"[MCP glasses_video_regenerate] Complete. {len(result.get('videos', []))} videos")
    return result
