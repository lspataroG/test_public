...
Only return the JSON object, nothing else."""
    ...
    for i, img in enumerate(garment_images_bytes_list):
        ...
        pieces.append(img)
    pieces.append("Classify this garment and determine the view for each image. For the view classification, carefully analyze the garment construction to determine front vs back — do not guess based on first impression.")
...
Only return the JSON object, nothing else."""
    ...
    try:
        ...
        valid_indices = [i for i in raw_indices if isinstance(i, int) and 0 <= i < num_images]
        ...
        valid_indices.sort(key=lambda i: 0 if eval_by_index.get(i, {}).get("angle") == "perfect" else 1)
        ...
        return {"best_indices": [], "evaluations": [], "view_details": "", "rejected_views": []}
...
def select_best_front(
    ...
    logger.debug(f"[Fitting] Selecting best front images from {len(garment_images)} candidates")
    result = _select_best_view(client, garment_images, "front", model, category=category)
    logger.debug(f"[Fitting] Best front: indices={result.get('best_indices')}")
    for ev in result.get("evaluations", []):
        logger.debug(f"[Fitting]   IMAGE_{ev.get('index')}: angle={ev.get('angle')}, quality={ev.get('quality')}")
    return result
...
"""
    ...
    for i, img in enumerate(back_images):
        ...
        pieces.append(img)
    pieces.append("Identify the brand and product, and refine the detail lists for both views.")
...
def select_best_back(
    ...
    logger.debug(f"[Fitting] Selecting best back images from {len(garment_images)} candidates")
