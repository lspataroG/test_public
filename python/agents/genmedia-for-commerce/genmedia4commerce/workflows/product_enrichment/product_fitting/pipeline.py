...
from .garment_eval import evaluate_garments, evaluate_footwear, evaluate_wearing_quality
...
from .generation import generate_fitting, generate_fitting_back_from_front, fix_fitting

from workflows.shared.nano_banana import NANO_TIMEOUT_SECONDS

logger = logging.getLogger(__name__)

# fix_tradeinn uses full_body/upper_body/lower_body framing, but model photos
# are named front_top/front_bottom. This maps framing to model photo key suffix.
...
async def _evaluate_image(
    ...
    if category == "footwear":
        ...
        garments_eval = await run_in_threadpool(
            evaluate_footwear, genai_client, image, garment_imgs,
            "gemini-3-flash-preview", footwear_desc,
        )
        wearing_eval = {"score": 3, "explanation": "Included in footwear eval"}
    else:
        garments_eval, wearing_eval = await asyncio.gather(
            run_in_threadpool(
                evaluate_garments, genai_client, image, garment_imgs,
                "gemini-3-flash-preview", eval_view_details, desc_general,
            ),
            run_in_threadpool(
                evaluate_wearing_quality, genai_client, image, "gemini-3-flash-preview",
            ),
        )
...
async def _try_fix(
    ...
    *,
    ...
    product_id: Optional[str] = None,
    category: str = "",
    wearing_eval: Optional[dict] = None,
):
    """Try to fix an image and evaluate the fix. Returns (fixed_result_or_None, fix_log)."""
    request_logger = logging.LoggerAdapter(logger, {"product_id": product_id or "unknown"})
    details = garments_eval.get("garment_details", [])
    feedback_parts = [f"Garment {i}: score={d['score']}/10 -- {d['explanation']}" for i, d in enumerate(details)]
    if wearing_eval and wearing_eval.get("score", 3) <= 1:
        feedback_parts.append(f"Wearing quality: score={wearing_eval['score']}/3 -- {wearing_eval.get('explanation', 'Poor wearing quality')}")
    eval_feedback = "\n".join(feedback_parts)

    try:
        fixed_image = await run_in_threadpool(
            fix_fitting, nano_client, gen_message, gen_config,
            image, eval_feedback, generation_model, nano_timeout,
        )
        if fixed_image is None:
            request_logger.debug(f"[Fitting] {view_name} fix -> FAILED (no image)")
            return None, {"status": "failed"}

        fixed_eval, fixed_wearing, fixed_details, fixed_score, fixed_discarded = await _evaluate_image(
            genai_client, view_name, fixed_image, garment_imgs, eval_view_details, desc_general,
            product_id=product_id, category=category,
        )
        for di, d in enumerate(fixed_details):
            request_logger.debug(f"[Fitting] {view_name} fix garment {di}: score={d['score']}/10 -- {d['explanation']}")
        request_logger.info(f"[Fitting] {view_name} fix score={fixed_score:.0f}/100, discard={fixed_discarded}, wearing={fixed_wearing['score']}/3 -- {fixed_wearing.get('explanation', '')}")

        fix_log = {
            "garments_score": fixed_score,
            "garment_details": [{"score": d["score"], "explanation": d["explanation"]} for d in fixed_details],
...
async def _generate_view(
    ...
    *,
    ...
    best: Optional[dict] = None  # single best result (discarded or not)
    ...
    def _is_better(score, is_discarded):
        ...
        best_score = best.get("final_score") or best["garments_evaluation"]["garments_score"]
    ...
    def _update_best(image, garments_eval, wearing_eval, score, is_discarded, attempt_idx, gen_msg=None, gen_cfg=None):
        nonlocal best
        if not _is_better(score, is_discarded):
            return
        best = {
            "index": attempt_idx, "image": image,
    ...
    def _emit_sse(index, image, garments_eval, is_discarded, score, fix=False, final_fix=False):
        event = {
            "index": index, "view": view_name,
    ...
    fix_kwargs = dict(
        nano_client=nano_client, genai_client=genai_client,
        generation_model=generation_model, nano_timeout=nano_timeout,
        eval_view_details=eval_view_details, desc_general=desc_general,
        product_id=product_id, category=category,
    )

    for attempt in range(max_retries):
        request_logger.info(f"[Fitting] {view_name} attempt {attempt + 1}/{max_retries}")
        ...
        try:
            if gen_func is not None:
                fitting_image, gen_message, gen_config = await run_in_threadpool(gen_func, **gen_kwargs)
            else:
                fitting_image, gen_message, gen_config = await run_in_threadpool(
                    generate_fitting, nano_client, scenario, garment_imgs,
                    model_img, framing, generation_model, view_name,
                    garment_description, gender, nano_timeout,
                )
        except Exception as e:
            request_logger.error(f"[Fitting] {view_name} attempt {attempt + 1} generation error: {e}")
            fitting_image = None

        if fitting_image is None:
            request_logger.info(f"[Fitting] {view_name} attempt {attempt + 1} -> FAILED (no image)")
            sse_events.append(
                f"data: {json.dumps({'index': attempt, 'view': view_name, 'status': 'failed', 'error': 'Generation failed'})}\n\n"
            )
            attempts_log.append({"attempt": attempt, "status": "failed", "generation": None, "fix": None, "winner": None})
            continue

        # -- Evaluate --
        garments_eval, wearing_eval, details, score, is_discarded = await _evaluate_image(
            genai_client, view_name, fitting_image, garment_imgs, eval_view_details, desc_general,
            product_id=product_id, category=category,
        )
        for di, d in enumerate(details):
            request_logger.debug(f"[Fitting] {view_name} attempt {attempt + 1} garment {di}: score={d['score']}/10 -- {d['explanation']}")
        request_logger.info(f"[Fitting] {view_name} attempt {attempt + 1}/{max_retries} score={score:.0f}/100, discard={is_discarded}, wearing={wearing_eval['score']}/3 -- {wearing_eval.get('explanation', '')}")

        gen_eval_summary = {
            "garments_score": score,
            "garment_details": [{"score": d["score"], "explanation": d["explanation"]} for d in details],
        ...
        _update_best(fitting_image, garments_eval, wearing_eval, score, is_discarded, attempt, gen_message, gen_config)

        if _is_good_enough(score, is_discarded):
            attempts_log.append({"attempt": attempt, "status": "accepted", "generation": gen_eval_summary, "fix": None, "winner": "generation"})
            request_logger.info(f"[Fitting] {view_name} attempt {attempt + 1} -> good score, done")
            break

        # -- Fix this attempt --
        request_logger.info(f"[Fitting] {view_name} attempt {attempt + 1}/{max_retries} trying fix")
        fix_result, fix_log = await _try_fix(
            view_name, fitting_image, garment_imgs, garments_eval,
            gen_message=gen_message, gen_config=gen_config, wearing_eval=wearing_eval, **fix_kwargs,
        )
        ...
        if fix_result is not None:
            ...
            _emit_sse(attempt, fix_result["image"], fix_result["garments_evaluation"], fixed_discarded, fixed_score, fix=True)

            if _is_better(fixed_score, fixed_discarded):
                request_logger.info(f"[Fitting] {view_name} attempt {attempt + 1}/{max_retries} fix improved: {score:.0f} -> {fixed_score:.0f} (discard: {is_discarded} -> {fixed_discarded})")
                _update_best(fix_result["image"], fix_result["garments_evaluation"], fix_result["wearing_evaluation"], fixed_score, fixed_discarded, attempt, gen_message, gen_config)
                winner = "fix"
            else:
                request_logger.info(f"[Fitting] {view_name} attempt {attempt + 1}/{max_retries} fix no improvement: {score:.0f} -> {fixed_score:.0f} (discard: {is_discarded} -> {fixed_discarded})")

            if _is_good_enough(fixed_score, fixed_discarded):
                attempts_log.append({"attempt": attempt, "status": "accepted", "generation": gen_eval_summary, "fix": fix_log, "winner": winner})
                request_logger.info(f"[Fitting] {view_name} attempt {attempt + 1} -> fix good score, done")
                break

        attempts_log.append({"attempt": attempt, "status": "discarded" if is_discarded else "accepted", "generation": gen_eval_summary, "fix": fix_log, "winner": winner})

    # -- Final fix: if best across all attempts < 90, try one last fix on the overall best --
    if best is not None and _best_score() < 90:
        request_logger.info(f"[Fitting] {view_name}: all {max_retries} attempts done, best score={_best_score():.0f}, trying final fix")
        fix_result, fix_log = await _try_fix(
            view_name, best["image"], garment_imgs, best["garments_evaluation"],
            gen_message=best["gen_message"], gen_config=best["gen_config"],
            wearing_eval=best.get("wearing_evaluation"), **fix_kwargs,
        )
        if fix_result is not None and _is_better(fix_result["score"], fix_result["is_discarded"]):
            request_logger.info(f"[Fitting] {view_name} final fix improved: {_best_score():.0f} -> {fix_result['score']:.0f}")
            _update_best(fix_result["image"], fix_result["garments_evaluation"], fix_result["wearing_evaluation"], fix_result["score"], fix_result["is_discarded"], best["index"], best["gen_message"], best["gen_config"])
            _emit_sse(best["index"], fix_result["image"], fix_result["garments_evaluation"], fix_result["is_discarded"], fix_result["score"], fix=True, final_fix=True)
            attempts_log.append({"attempt": "final_fix", "status": "improved", "fix": fix_log, "winner": "fix"})
        else:
            request_logger.info(f"[Fitting] {view_name} final fix did not improve")
            attempts_log.append({"attempt": "final_fix", "status": "no_improvement", "fix": fix_log, "winner": "generation"})

    # Log final outcome
    if best is None:
        request_logger.info(f"[Fitting] {view_name}: all attempts failed, no image produced")
    elif best.get("final_score") is None:
        disc_score = best["garments_evaluation"]["garments_score"]
        request_logger.info(f"[Fitting] {view_name}: best was discarded (score={disc_score:.0f}), using as fallback")
    else:
        request_logger.info(f"[Fitting] {view_name}: final score={best['final_score']:.0f}")

    return best, sse_events, attempts_log


def _build_validation(result: Optional[dict]) -> Optional[dict]:
...
async def run_fitting_pipeline(
    ...
    classification = await run_in_threadpool(
        classify_garments, genai_client, garment_images_bytes,
    )

    category = classification["category"]
    description = classification.get("description", "")
    views = classification.get("views", [])
    request_logger.info(f"[Fitting] Classification: category={category}, description='{description}'")
    ...
    if category in ("footwear", "head_accessory"):
        request_logger.info(f"[Fitting] {category} detected: selecting best 2 from all {len(garment_images_bytes)} images")
        front_selection = await run_in_threadpool(select_best_front, genai_client, garment_images_bytes, category=category)
        front_indices = [i for i in front_selection.get("best_indices", []) if i < len(garment_images_bytes)]
        back_indices = []
        ...
        front_candidate_images = [garment_images_bytes[i] for i in front_candidate_indices]
        back_candidate_images = [garment_images_bytes[i] for i in back_candidate_indices]
        request_logger.debug(f"[Fitting] Front candidates: {front_candidate_indices}, Back candidates: {back_candidate_indices}")
        ...
        if has_front_candidates:
            selection_tasks.append(run_in_threadpool(select_best_front, genai_client, front_candidate_images, category=category))
        if has_back_candidates:
            selection_tasks.append(run_in_threadpool(select_best_back, genai_client, back_candidate_images, category=category))

        selection_results = await asyncio.gather(*selection_tasks) if selection_tasks else []
        ...
        if has_front_candidates:
            ...
            result_idx += 1
            front_indices = [front_candidate_indices[i] for i in front_selection.get("best_indices", []) if i < len(front_candidate_indices)]
        else:
            front_selection = {"best_indices": [], "evaluations": [], "rejected_views": []}
            front_indices = []

        if has_back_candidates:
            back_selection = selection_results[result_idx]
            back_indices = [back_candidate_indices[i] for i in back_selection.get("best_indices", []) if i < len(back_candidate_indices)]
        else:
            back_selection = {"best_indices": [], "evaluations": [], "rejected_views": []}
            back_indices = []
    ...
    request_logger.debug(f"[Fitting] Selected front indices: {front_indices}, back indices: {back_indices}")
    request_logger.debug(f"[Fitting] Front garments: {len(front_garments)}, Back garments: {len(back_garments)}")
    ...
    if not front_garments and not back_garments:
        ...
        if front_rejected or back_rejected:
            ...
            error_msg = (
                "No usable front or back view found. " + " ".join(parts)
            )
        else:
    ...
    if front_garments:
        ...
        request_logger.debug(f"[Fitting] Will generate front view ({len(front_garments)} ref images, indices {front_indices}) using model photo: {model_photo_key}")
    if back_garments:
        request_logger.debug(f"[Fitting] Back garments available ({len(back_garments)} ref images, indices {back_indices}) -- will generate after front")
    ...
    garment_description = await run_in_threadpool(
        describe_garment_detailed, genai_client, front_garments, back_garments,
    )
    front_view_details = garment_description.get("front_details", "")
    back_view_details = garment_description.get("back_details", "")
    request_logger.debug(f"[Fitting] Garment description: general='{garment_description.get('general', '')}'")
    request_logger.debug(f"[Fitting] Garment description: front_details='{front_view_details}'")
    request_logger.debug(f"[Fitting] Garment description: back_details='{back_view_details}'")
    ...
    if not has_front and has_back:
        ...
        request_logger.debug(f"[Fitting] No front garments -- generating back from front model photo ({model_photo_key})")
        ...
        back_result, back_events, back_attempts_log = await _generate_view(
            "back", back_garments, None, max_retries,
            gen_func=generate_fitting_back_from_front, gen_kwargs=gen_kwargs,
            **shared_kwargs,
        )
        ...
        if has_front:
            request_logger.info(f"[Fitting] Phase 1: Generating front (up to {max_retries} attempts)")
            front_result, front_events, front_attempts_log = await _generate_view(
                "front", front_garments, front_model_photo, max_retries,
                **shared_kwargs,
            )
        ...
        if best_front_image:
            status = "ready" if front_result.get("final_score") is not None else "discarded"
            request_logger.debug(f"[Fitting] Best front image: attempt {front_result['index']} (status={status})")
        else:
            request_logger.debug("[Fitting] No front image produced (all failed)")

        if has_back and best_front_image is not None:
            request_logger.info(f"[Fitting] Phase 2: Generating back (up to {max_retries} attempts) from best front")
            gen_kwargs = {
                "client": nano_client,
                "scenario": scenario,
                "back_garment_images": back_garments,
            ...
            request_logger.debug("[Fitting] Skipping back generation -- no valid front image available")

    # Build response
    def _format_side(result: Optional[dict], attempts: list[dict]) -> Optional[dict]:
        if result is None:
            return None
