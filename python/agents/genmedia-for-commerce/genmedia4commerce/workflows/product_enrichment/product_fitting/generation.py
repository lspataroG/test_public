...
from workflows.shared.nano_banana import generate_nano, NANO_TIMEOUT_SECONDS
from workflows.shared.debug_utils import save_debug_image

logger = logging.getLogger(__name__)


def generate_fitting(client, scenario, garment_images, preprocessed_model, framing="full_body", model="gemini-3.1-flash-image-preview", garment_view="front", garment_description=None, model_gender=None, timeout=NANO_TIMEOUT_SECONDS):
    """
    ...
    """
    logger.debug(f"[Fitting] Starting product fitting generation (framing: {framing}, model: {model}, garment_view: {garment_view}, num_ref_images: {len(garment_images)})")

    if framing == "upper_body":
        system_prompt, user_task, aspect_ratio = _get_upper_body_fitting_prompt(scenario, garment_view, garment_description, model_gender)
    elif framing == "head":
        system_prompt, user_task, aspect_ratio = _get_head_fitting_prompt(scenario, garment_view, garment_description, model_gender)
    elif framing == "footwear":
        system_prompt, user_task, aspect_ratio = _get_footwear_fitting_prompt(scenario, garment_view, garment_description, model_gender)
    elif framing == "lower_body":
        system_prompt, user_task, aspect_ratio = _get_lower_body_fitting_prompt(scenario, garment_view, garment_description, model_gender)
    else:
        system_prompt, user_task, aspect_ratio = _get_full_body_fitting_prompt(scenario, garment_view, garment_description, model_gender)
    ...
    if len(garment_images) == 1:
        ...
        user_message.append(f"### PRODUCT TO FIT ON THE MODEL ({view_label}) — {len(garment_images)} reference images of the SAME product from different angles: ")
    ...
    result = generate_nano(client, user_message, model=model, config=config, timeout=timeout)
...
def _get_full_body_fitting_prompt(scenario, garment_view="front", garment_description=None, model_gender=None):
    ...
    view_str = "from behind (back view)" if garment_view == "back" else "from the front, facing the camera"
...
def _get_upper_body_fitting_prompt(scenario, garment_view="front", garment_description=None, model_gender=None):
...
def _get_head_fitting_prompt(scenario, garment_view="front", garment_description=None, model_gender=None):
...
def _get_footwear_fitting_prompt(scenario, garment_view="front", garment_description=None, model_gender=None):
...
def _get_lower_body_fitting_prompt(scenario, garment_view="front", garment_description=None, model_gender=None):
...
def generate_fitting_back_from_front(client, scenario, back_garment_images, best_front_image, framing="full_body", model="gemini-3.1-flash-image-preview", garment_description=None, model_gender=None, timeout=NANO_TIMEOUT_SECONDS):
    """
    ...
    Returns:
        tuple: (image_bytes, user_message, config) or (None, None, None) if failed
    """
    logger.debug(f"[Fitting] Starting back-from-front generation (framing: {framing}, model: {model}, num_back_refs: {len(back_garment_images)})")

    if framing == "upper_body":
        system_prompt, user_task, aspect_ratio = _get_upper_body_back_from_front_prompt(scenario, garment_description, model_gender)
    elif framing == "lower_body":
        system_prompt, user_task, aspect_ratio = _get_lower_body_back_from_front_prompt(scenario, garment_description, model_gender)
    else:
        system_prompt, user_task, aspect_ratio = _get_full_body_back_from_front_prompt(scenario, garment_description, model_gender)
    ...
    if len(back_garment_images) == 1:
        user_message.append("### BACK OF THE PRODUCT (reference for the garment's back appearance): ")
        user_message.append(back_garment_images[0])
    else:
        user_message.append(f"### BACK OF THE PRODUCT -- {len(back_garment_images)} reference images of the SAME product's back: ")
...
def fix_fitting(client, original_message, original_config, generated_image,
                eval_feedback, model, timeout=NANO_TIMEOUT_SECONDS):
    """
...
Fix ONLY the issues described above. Keep everything else identical — same pose, body, composition, setting, complementary garments. Reproduce the garment details exactly as shown in the reference images. Generate the corrected image.""",
    ...
    result = generate_nano(client, fix_message, model=model, config=original_config, timeout=timeout)
...
def _get_full_body_back_from_front_prompt(scenario, garment_description=None, model_gender=None):
...
def _get_upper_body_back_from_front_prompt(scenario, garment_description=None, model_gender=None):
...
def _get_lower_body_back_from_front_prompt(scenario, garment_description=None, model_gender=None):
