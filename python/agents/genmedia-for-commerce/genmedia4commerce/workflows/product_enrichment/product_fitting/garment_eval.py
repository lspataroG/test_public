...
Only return the JSON object, nothing else."""
    ...
    try:
        ...
        return {"explanation": f"Error during garment evaluation: {str(e)}", "score": 0}
...
Only return the JSON object, nothing else."""
    ...
    pieces = []
    pieces.append("GENERATED IMAGE (this is the AI-generated image you must evaluate — it shows a full-body model wearing the shoes, look at the feet):")
    pieces.append(generated_image_bytes)
    pieces.append(f"REFERENCE IMAGES ({num_refs} product photos of the original shoe — compare the generated shoes above against these):")
    for i, ref_img in enumerate(reference_images):
        if num_refs > 1:
            pieces.append(f"REFERENCE {i + 1}:")
        pieces.append(ref_img)
    pieces.append("IMPORTANT: The FIRST image above is the generated one (full-body model shot). All subsequent images are references. Zoom into the FEET area of the first image and compare the shoes there against the reference product photos. Do NOT confuse the reference images with the generated image.")
    ...
    try:
        ...
        result = {"explanation": f"Error during footwear evaluation: {str(e)}", "score": 0}
...
Only return the JSON object, nothing else."""
    ...
    try:
        ...
        return {
            "explanation": f"Error during wearing quality evaluation: {str(e)}",
            "score": 0,
        }
