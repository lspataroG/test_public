...
# Project imports
from workflows.shared.llm_utils import get_part, get_generate_content_config, retry_with_exponential_backoff
...
def generate_nano(client, text_images_pieces, model="gemini-3.1-flash-image-preview", config=None, timeout=NANO_TIMEOUT_SECONDS):
    """
    ...
    if timeout is None:
        logger.debug(f"[generate_nano] Thread {thread_id}: Calling generate_content (no timeout)...")
        result = _call_api()
    else:
        ...
        logger.debug(f"[generate_nano] Thread {thread_id}: Calling generate_content (timeout: {timeout}s)...")
        executor = ThreadPoolExecutor(max_workers=1)
        try:
            future = executor.submit(_call_api)
            result = future.result(timeout=timeout)
        except FuturesTimeoutError:
            logger.error(f"[generate_nano] Thread {thread_id}: API call timed out after {timeout}s")
