...
def upload_folder_to_gcs(
    ...
    logger.info(f"Preparing to upload {len(file_paths)} files using {max_workers} workers...")
    ...
    logger.info(f"✓ Successfully uploaded {len(uploaded_uris)} files to gs://{bucket_name}/{destination_prefix}")
...
def download_file_from_gcs(
    ...
    logger.info(f"✓ Downloaded: gs://{bucket_name}/{source_blob_name} → {destination_file_path}")
...
def save_and_upload_to_gcs(
    ...
    if pre_sampled_frames is None:
        raise ValueError("pre_sampled_frames is required - frame extraction must be done before calling this function")
    ...
    try:
        ...
        for idx, frame_bytes in enumerate(pre_sampled_frames):
            with open(video_frames_dir / f"{product_id}-ld-{idx:04d}.{image_format}", 'wb') as f:
                f.write(frame_bytes)
        ...
        for clip in result['clips']:
            ...
            if 'reference_images_bytes' in clip:
                ...
                for i, b in enumerate(clip['reference_images_bytes']):
                    with open(ref_dir / f"reference_{i:02d}.png", 'wb') as f: f.write(b)
                ...
                if 'reference_images_unstacked_bytes' in clip:
                    ...
                    for i, b in enumerate(clip['reference_images_unstacked_bytes']):
                        with open(uns_dir / f"reference_unstacked_{i:02d}.png", 'wb') as f: f.write(b)
            else:
                with open(clip_path / "start_frame.png", 'wb') as f: f.write(clip['start_frame_bytes'])
                with open(clip_path / "end_frame.png", 'wb') as f: f.write(clip['end_frame_bytes'])

            # Save clip frames (from frame_list in clip result)
            clip_frames = clip.get('frame_list', [])
            for i, b in enumerate(clip_frames):
                with open(clip_path / "frames" / f"frame_{i:04d}.png", 'wb') as f: f.write(b)

            # Metadata building
            meta = {k: v for k, v in clip.items() if not k.endswith('bytes') and k != 'frame_list'}
        ...
        gcs_prefix = f"{gcs_destination_prefix}/{product_id}" if gcs_destination_prefix else product_id
        ...
        manifest_data = _build_manifest(
            product_id, bucket_name, gcs_prefix, result, clips_metadata, pre_sampled_frames, temp_dir
        )
        ...
        manifest_uri = upload_file_to_gcs(
            bucket_name, str(manifest_path),
            f"{gcs_prefix}/gcs_manifest.json",
            project_id, "application/json"
        )
...
def _build_manifest(product_id, bucket, prefix, result, clips_meta, pre_sampled_frames, temp_dir):
    ...
    manifest = {
        ...
        "video": {
            ...
            "frames": [f"{base_uri}video/frames/{product_id}_Id_{i:04d}.png" for i in range(len(pre_sampled_frames))]
        },
        "clips": [],
    ...
    for clip, meta in zip(result['clips'], clips_meta):
        ...
        if 'sampled_frame_indices' in clip and 'frame_classifications' in clip:
            frame_map = dict(zip(clip['sampled_frame_indices'], clip['frame_classifications']))

        for i in range(num_frames):
            clip_entry["frames"].append([
                f"{clip_base}/frames/frame_{i:04d}.png",
                frame_map.get(i)
            ])
        ...
        if 'reference_images_bytes' in clip:
            ...
            for i in range(len(clip['reference_images_bytes'])):
                ...
                ref_uris.append([f"{clip_base}/reference_images/reference_{i:02d}.png", cls])
            ...
            if 'reference_images_unstacked_bytes' in clip:
                ...
                for i in range(len(clip['reference_images_unstacked_bytes'])):
                    ...
                    uns_uris.append([f"{clip_base}/reference_images_unstacked/reference_unstacked_{i:02d}.png", cls])
