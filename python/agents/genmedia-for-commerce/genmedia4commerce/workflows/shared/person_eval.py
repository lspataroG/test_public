...
def _mask_eye_regions(img_bgr: np.ndarray, face) -> np.ndarray:
    ...
    for eye_pt in [left_eye, right_eye]:
        ...
        cv2.ellipse(masked, center, axes, angle=0,
                    startAngle=0, endAngle=360,
                    color=skin_color, thickness=-1)
...
def get_eval_pool() -> ThreadPoolExecutor:
    ...
    if _eval_pool is None:
        logger.info(f"[Face Eval] Creating thread pool with {_POOL_MAX_WORKERS} workers")
        _eval_pool = ThreadPoolExecutor(max_workers=_POOL_MAX_WORKERS)
    return _eval_pool


def validate_model_photo(img_bytes: bytes, max_yaw: float = 30.0, max_pitch: float = 25.0) -> dict:
    """
    ...
    if hasattr(face, "pose"):
        ...
        logger.info(f"[Photo Validation] Head pose — yaw: {yaw:.1f}, pitch: {pitch:.1f}, roll: {roll:.1f}")
    ...
    if hasattr(face, "landmark_3d_68") and face.landmark_3d_68 is not None:
        ...
        logger.info(f"[Photo Validation] Eye Aspect Ratio — left: {left_ear:.3f}, right: {right_ear:.3f}, avg: {avg_ear:.3f}")
...
def get_face_embedding_insightface(
    ...
    if mask_eyes:
        ...
        if masked_faces:
            ...
            logger.debug(f"[InsightFace] Extracted masked embedding shape: {embedding.shape}")
            return embedding
        logger.warning("[InsightFace] No face detected after masking eyes, falling back to unmasked")
...
def compare_faces_insightface(
    ...
    ref_embedding = get_face_embedding_insightface(reference_face_bytes, mask_eyes=mask_eyes)
    gen_embedding = get_face_embedding_insightface(generated_face_bytes, mask_eyes=mask_eyes)

    if ref_embedding is None or gen_embedding is None:
        logger.warning("[InsightFace] Could not extract embeddings from one or both images")
        return {
...
def evaluate_person_match(reference_face_bytes, generated_vto_bytes, mask_eyes=False):
    ...
    try:
        result = compare_faces_insightface(reference_face_bytes, generated_face_bytes, mask_eyes=mask_eyes)
        result["face_detected"] = result.get("embeddings_extracted", False)

        similarity = result['similarity_percentage']
        logger.info(f"[InsightFace Eval] Evaluation complete - Similarity: {similarity:.2f}% [{_now()}]")
...
def submit_evaluation(reference_face_bytes: bytes, generated_vto_bytes: bytes, mask_eyes: bool = False):
    """
    ...
    """
    pool = get_eval_pool()
    return pool.submit(evaluate_person_match, reference_face_bytes, generated_vto_bytes, mask_eyes)
