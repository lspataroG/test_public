...
def predict_parallel(to_predict, predict_function, max_workers=8, show_progress_bar=True):
    """
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as pool:
        if show_progress_bar:
            for result in tqdm(pool.map(predict_function, to_predict), total=len(to_predict)):
