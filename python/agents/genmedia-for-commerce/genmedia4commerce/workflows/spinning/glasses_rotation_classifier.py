    ...
    try:
        ...
        while True:
            ...
            if frame_idx % SAMPLE_INTERVAL == 0:
                ...
                try:
                    mask_bytes = get_background_mask_vertex(segmentation_client, frame_bytes)
        ...
        if len(centroids_x) < 6:
            logger.warning(f"[Glasses Rotation] Too few valid frames ({len(centroids_x)})")
            return "invalid"
        ...
        if ratio < 0.6:
            logger.info(f"[Glasses Rotation] Ambiguous: cw={cw_score:.1f}, acw={acw_score:.1f}")
            return "invalid"
