...
def classify_rotation_detailed(
    ...
    if detection["error"]:
        return {
            ...
            "raw_segments": [],
            "filtered_segments": [],
            "dx_max": 0, "dx_min": 0, "dy_max": 0, "dy_min": 0,
        }
...
def _apply_v3f_rules(detection: Dict, config: ClassifierConfig) -> tuple:
    ...
    if total_frames > 0 and cw_frames > 0 and acw_frames > 0:
        ...
        if minority_ratio > config.min_ratio and minority_avg_dx > config.min_avg_dx:
            return "invalid", f"DIR_CHANGE: ratio={minority_ratio:.2f}, strength={minority_avg_dx:.2f}"
...
def c_detect_rotation_segments(video_path: str, config: ClassifierConfig) -> Dict:
    ...
    while True:
        ...
        if p1 is not None:
            ...
            if len(good_new) > 0:
                ...
                mask_dx = (np.abs(dx_values) > 0.5) & (np.abs(dy_values) < np.abs(dx_values))
                if np.sum(mask_dx) > 0:
    ...
    if len(filtered_segments) == 0:
        if len(raw_segments) > 0:
            strongest = max(raw_segments, key=lambda s: abs(s["avg_dx"]) * s["duration"])
