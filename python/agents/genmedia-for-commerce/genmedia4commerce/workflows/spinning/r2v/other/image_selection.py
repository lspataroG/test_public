...
Only return the JSON object, nothing else."""
    ...
    for i, img in enumerate(images_bytes):
        ...
        pieces.append(img)
    pieces.append(f"Classify the product type and view angle/quality of all {num_images} images.")
    ...
    try:
        ...
        return {
            "product_type": "other",
            "classifications": [{"index": i, "view": "other", "quality": 5} for i in range(num_images)],
        }
...
def select_best_images(
    ...
    logger.info(
        ...
        + ", ".join(f"IMG_{c['index']}={c['view']}(q{c['quality']})" for c in classifications)
    )
    ...
    logger.info(f"[R2V Selection] Selected {len(selected)} from {len(images_bytes)} images")
    return selected
