...
def sort_masks_by_horizontal_position(mask_data_list: list,
                                      mode: str = 'right',
    ...
    if mode not in ['right', 'left']:
        if verbose: print(f"Warning: Invalid mode '{mode}'. Defaulting to 'right'.")
        mode = 'right'
    ...
    try:
        ...
        if verbose: print("Warning: Could not determine mask dimensions. Skipping sort.")
        return mask_data_list
        
    if img_w == 0:
        if verbose: print("Warning: Mask width is 0. Skipping sort.")
        return mask_data_list
    ...
    def calculate_horizontal_score(mask_data_tuple: tuple) -> float:
        ...
        try:
            ...
            if verbose: print(f"Warning: Could not process a mask. Error: {e}")
            return -float('inf') if mode == 'right' else float('inf')
    ...
    sorted_data = sorted(mask_data_list,
                         key=calculate_horizontal_score,
                         reverse=sort_reverse)
    
    if verbose: print(f"Sorted {len(sorted_data)} mask/data tuples by '{mode}most' score.")
    return sorted_data
...
def divide_duplicate_image(image_bytes: bytes, client, verbose: bool = False, return_masks=False) -> list:
    """
    ...
    masks = [
        (np.array(mask.mask._pil_image.convert("L")) > 0, round(float(mask.labels[0].score), 5)) 
    ...
    if verbose: 
        print(f'Calculated threshold {threshold}, Number of masks selected  {len(masks)}')
