...
# Project imports
from workflows.spinning.r2v.shoes.classify_shoes import classify_shoe, classify_shoe_closure
from workflows.spinning.r2v.shoes.images_utils import image_closure_selection, sample_and_process_frames
from workflows.spinning.r2v.shoes.product_consistency_validation import validate_product_consistency
...
from workflows.spinning.r2v.shoes.video_validation_r2v import validate_and_fix_product_spin_consistency_r2v
...
def generate_single_clip_r2v(
    ...
    try:
        logger.info(f"Clip {clip_idx}: Reference-to-Video with {len(reference_images)} reference images")

        # Generate prompt if not provided
        if not veo_prompt:
            logger.info(f"  Generating R2V prompt...")
        ...
        if veo_prompt is None:
            raise Exception("Prompt generation failed. The Gemini service may be temporarily unavailable.")
        ...
        while retry_count < max_retries:
            ...
            if video_bytes:
                try:
                    is_valid, reason, new_video_bytes, frame_classifications, sampled_indices, total_frames, frame_list = validate_and_fix_product_spin_consistency_r2v(video_bytes, client, shoe_classifier_model)
                except Exception as validation_err:
                    # Transient errors (SSL, network) during validation — retry
                    retry_count += 1
                    logger.warning(f"  ✗ Validation crashed (attempt {retry_count}/{max_retries}): {type(validation_err).__name__}: {validation_err}")
                ...
                if is_valid:
                    ...
                    if validate_product_consistency_flag:
                        logger.info(f'Validating product consistency...')
                        try:
                            consistency_valid, consistency_msg, _ = validate_product_consistency(
                            ...
                            retry_count += 1
                            logger.warning(f"  ✗ Product consistency check crashed (attempt {retry_count}/{max_retries}): {type(consistency_err).__name__}: {consistency_err}")
                            if retry_count >= max_retries:
                                raise
                            continue
                        ...
                        if not consistency_valid:
                            retry_count += 1
                            logger.warning(f" Product consistency validation failed: {consistency_msg} (attempt {retry_count}/{max_retries})")
                            if retry_count >= max_retries:
                            ...
                            logger.info(f"Product consistency validation passed: {consistency_msg}")
                    ...
                    retry_count += 1
                    logger.info(f"  ✗ Video generation not valid: {reason} (attempt {retry_count}/{max_retries})")
                ...
                retry_count += 1
                logger.warning(f"  ✗ Video generation failed (attempt {retry_count}/{max_retries})")
...
def preprocess_classify_images(
    ...
    logger.info(f"✓ Initial classification complete: {len(initial_predictions)} predictions. Predictions {initial_predictions}")

    images_to_split = [
        img_bytes for img_bytes, pred in zip(images_bytes_list, initial_predictions)
    ...
    single_images_pred = [
        pred.strip().lower() for _, pred in zip(images_bytes_list, initial_predictions)
        if pred.strip().lower() not in ["multiple", "invalid"]
    ]
    ...
    if len(images_to_split) > 0:
        logger.info(f"Step 1.2: Splitting {len(images_to_split)} images with multiple products...")
        split_results = predict_parallel(
            images_to_split,
            lambda img_bytes: divide_duplicate_image(img_bytes, client, return_masks=False),
            max_workers=num_workers,
            show_progress_bar=False
        )
        split_images_bytes_list = [
            image_bytes for splitted_result_list in split_results
            for image_bytes in splitted_result_list
        ]
        logger.info(f"Step 1.2: Classifying {len(split_images_bytes_list)} splitted images...")
        ...
        splitted_valid = [
            img_bytes for img_bytes, pred in zip(split_images_bytes_list, splitted_predictions)
            if pred.strip().lower() not in ["multiple", "invalid"]
        ]
        splitted_pred = [
            pred.strip().lower() for _, pred in zip(split_images_bytes_list, splitted_predictions)
            if pred.strip().lower() not in ["multiple", "invalid"]
        ]
    ...
    if video_gen_status == "exclude":
        ...
        return list(zip(all_valid_images, final_classifications)), video_gen_status, False

    # Velcro check before upscaling to skip expensive processing
    selected_closure_images = image_closure_selection(all_valid_images, final_classifications)
    has_velcro = False
    if selected_closure_images and len(selected_closure_images) > 0:
        ...
        if has_velcro:
            ...
            return list(zip(all_valid_images, final_classifications)), video_gen_status, True

    if upscale_images:
        # Extract, upscale, and clean
        all_valid_images = predict_parallel(
            all_valid_images,
            lambda img_bytes: extract_upscale_product(client, upscale_client, img_bytes, clean_after_upscale=True),
            max_workers=num_workers,
            show_progress_bar=False
        )
    ...
    return list(zip(all_valid_images, final_classifications)), video_gen_status, has_velcro
...
def run_video_gen_pipeline_r2v(
    ...
    try:
        logger.info(f"=== R2V PIPELINE STARTED ===")
        ...
        if video_gen_status == "exclude":
            raise ValueError(f"Cannot generate video with these images. Status: {video_gen_status}")
        ...
        reference_images, stacked_classes = stack_and_canvas_images(images_picked, ordered_classes)
        ...
        frame_list = clip_result["frame_list"]  # Get frames from validation (already extracted)

        logger.info(f"✓ R2V video generated ({len(video_bytes):,} bytes, {len(video_bytes)/1024/1024:.2f} MB)")
        ...
        logger.info(f"✓ Processed {len(sampled_frames_resized)} frames (resized to 1000x1000)")
        logger.info("=== R2V PIPELINE COMPLETE ===")
        logger.info(f"Final video size: {len(video_bytes):,} bytes ({len(video_bytes)/1024/1024:.2f} MB)")
        ...
        if gcs_bucket:
            ...
            clip_result["reference_images_classifications"] = stacked_classes  # Classifications for stacked images
            clip_result["reference_images_unstacked_bytes"] = images_bytes  # Save unstacked reference frames
            clip_result["reference_images_unstacked_classifications"] = ordered_classes  # Classifications for unstacked images
            ...
            logger.info(f"✓ Uploaded {len(uploaded_files)} files to gs://{gcs_bucket}/{gcs_destination_prefix}/{product_id}/")

            # Return GCS metadata for endpoint response (exclude large byte fields, but keep classifications)
            clip_metadata_clean = {
                k: v for k, v in clip_result.items()
                if k not in ('video_bytes', 'start_frame_bytes', 'end_frame_bytes', 'reference_images_bytes', 'reference_images_unstacked_bytes')
            }
