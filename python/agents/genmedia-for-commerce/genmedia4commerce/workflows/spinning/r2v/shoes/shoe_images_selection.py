...
def get_non_white_pixel_count(img_bytes: bytes) -> int:
    ...
    try:
        ...
        is_white = (img_array[:, :, 0] == 255) & (img_array[:, :, 1] == 255) & (img_array[:, :, 2] == 255)
...
def filer_top_four_views(selected_images):
  ...
  priority_orders = [
    ...
    ['front', 'front_left', 'front_right', 'top_front'],  # 3rd: best front (top_front as last resort)
...
def pick_images_by_ordered_best_side(images_classified: List[Tuple[bytes, str]]) -> List[Tuple[any, str]]:
    """
