...
# Project imports
from workflows.spinning.r2v.shoes.classify_shoes import classify_shoe, _is_endpoint_available
...
def is_valid_path(video_path, allowed_path):
    ...
    for i in range(len(video_path) - 1):
        ...
        if current_node not in allowed_path:
            return False, f"Invalid node found at index {i}: '{current_node}' is not in the map."

        # 2. Check if the transition to the next node is allowed
        if next_node not in allowed_path[current_node]:
            return False, f"Invalid transition at index {i}: cannot go from '{current_node}' to '{next_node}'"
    ...
    for i in range(len(video_path)):
        ...
        if node not in allowed_path:
            ...
            if consecutive_violations > max_consecutive_violations:
                return False, f"Too many consecutive violations ({consecutive_violations})", None
        ...
        if node in allowed_path[current_valid_node]:
            ...
            visited_nodes.add(node)
            consecutive_violations = 0
            consecutive_oscillations = 0  # Reset oscillation counter on valid forward movement
            corrected_path[i] = node
            continue
        ...
        for intermediate in allowed_path.get(current_valid_node, []):
            ...
            if node in allowed_path.get(intermediate, []) or node == intermediate:
                is_valid_skip = True
                violations_log.append(f"Skip transition at index {i}: '{current_valid_node}' -> '{node}' (via '{intermediate}')")
                consecutive_violations = 0
                corrected_path[i] = current_valid_node
                break
        ...
        consecutive_violations += 1
        violations_log.append(f"Invalid transition at index {i}: '{current_valid_node}' -> '{node}'")
        # Correct violation to current valid node
        corrected_path[i] = current_valid_node

        if consecutive_violations > max_consecutive_violations:
            return False, f"Too many consecutive violations ({consecutive_violations}) ending at index {i}: cannot go from '{current_valid_node}' to '{node}'", None

    if violations_log:
        return True, f"Path is valid with {len(violations_log)} misclassifications tolerated: {', '.join(violations_log)}", corrected_path
    else:
...
def is_valid(video_path, strict = True):
    ...
    if strict:
      ...
      is_valid_clockwise, clock_wise_message, corrected_path_clockwise = is_valid_path_lessstrict(video_path, allowed_path)
    ...
    if not is_valid_clockwise:
        ...
        if strict:
          is_valid_anticlockwise, anticlock_wise_message = is_valid_path(video_path[::-1], allowed_path)
          corrected_path_anticlockwise = None
        else:
          is_valid_anticlockwise, anticlock_wise_message, corrected_path_anticlockwise = is_valid_path_lessstrict(video_path[::-1], allowed_path)

        if is_valid_anticlockwise:
          return True, True, f'Anti Clockwise {anticlock_wise_message}', corrected_path_anticlockwise


    return False, False, f'Clockwise: {clock_wise_message}, AntiClockwise {anticlock_wise_message}', None
...
def all_classes_present(product_position_frames):
    class_order = ["right", "front_right","front", "front_left", "left", "back_left", "back", "back_right"]
    ...
    if missing_positions:
        return False, f"Missing required positions in video: {', '.join(missing_positions)}"
...
def frames_in_extremity_class(image_list, frame_classes, client, shoe_classifier_model, position='last'):
      """
...
def validate_and_fix_product_spin_consistency_r2v(
    ...
    if pre_classified_frames is not None:
        ...
        if _is_endpoint_available(model):
            sampled_frames, sampled_indices = sample_frames(frame_list=frame_list, target_samples_per_sec=12, original_fps=24)
        else:
            sampled_frames = frame_list
            sampled_indices = list(range(total_frames))

        product_position_frames = classify_frames(sampled_frames, client, model, mode='validation')

    # Filter out 'invalid' frames — they carry no path information (motion blur, transitions)
    if sampled_indices is not None:
        filtered = [(c, i) for c, i in zip(product_position_frames, sampled_indices) if c != 'invalid']
        if filtered:
    ...
    if not are_classes_present:
        return False, message_complete, video_bytes, product_position_frames, sampled_indices, total_frames, frame_list
    else:

      is_valid_spin, reversed, message_valid, corrected_path = is_valid(product_position_frames, strict = False)
      ...
      if not is_valid_spin:
          ...
          start_idx, end_idx = find_second_occurrence_range(product_position_frames, sampled_indices)
          if start_idx is None and end_idx is None:
              if 'front' == product_position_frames[0].lower() or 'front' == product_position_frames[-1].lower() or ('front' in product_position_frames[0].lower() and 'front' in product_position_frames[-1].lower()):
                    valid = False
                    message = f'Gap in front.'
              else:
                FIRST_AND_LAST_CLASS_FRAMES_THRESHOLD = 25

                number_frames_last_class = frames_in_extremity_class(frame_list,product_position_frames, client, model, position='last')
                number_frames_first_class = frames_in_extremity_class(frame_list,product_position_frames, client, model, position='first')
                combined_number_frames = number_frames_first_class + number_frames_last_class
          ...
          elif end_idx - start_idx < 3 and total_frames - end_idx < 3: #Makin sure the is at least two frames to pick from and that this happens close to the end
              message = 'Spin ends at the same class as initial and no trimming needed'
              valid = True
          else:
              if start_idx == end_idx:
                  cropped_index = start_idx
              else:
                  similar_image_index = find_most_similar_frame_index(frame_list[start_idx:end_idx], frame_list[0])
              ...
              total_frames = len(frame_list)
              filtered = [(idx, position) for idx, position in zip(sampled_indices, product_position_frames) if idx < cropped_index]
      ...
      return valid, message, new_video_bytes, product_position_frames, sampled_indices, total_frames, frame_list
