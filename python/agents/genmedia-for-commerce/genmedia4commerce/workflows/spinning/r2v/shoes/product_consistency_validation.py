...
def forward_fill_labels(
    ...
    filled_count = len(filled_labels) - sum(1 for label in frame_labels if label is not None and label)

    logger.info(f"Forward-filled {filled_count} null labels from {len(frame_labels)} sampled frames")
    logger.info(f"Result: {len(filled_labels)} frames with labels (expanded from {sum(1 for l in frame_labels if l)} labeled)")
...
def find_best_matches_with_ssim(
    ...
    """
    logger.info(f"Finding best matches using SSIM for each reference...")
    ...
    logger.info(f"Grouped {len(matched_pairs)} matched pairs into {len(pairs_by_reference)} reference groups")

    # Find best match for each reference
    frames_to_evaluate = []

    for ref_idx, pairs in pairs_by_reference.items():
        logger.info(f"  Reference {ref_idx}: Comparing {len(pairs)} candidate frames...")
        ...
        if frame_before_idx >= 0:
            ...
            logger.info(f"    Frame before: frame {frame_before_idx} (offset: -{offset_frames})")
        else:
        ...
        if frame_after_idx < len(all_frames):
            ...
            logger.info(f"    Frame after: frame {frame_after_idx} (offset: +{offset_frames})")
        else:
            logger.info(f"    Frame after: skipped (index {frame_after_idx} >= {len(all_frames)})")
...
def match_frames_to_references(
    ...
    for frame_idx, frame_label in zip(frame_indices, frame_labels):
        ...
        if clean_label in reference_lookup:
            ...
            logger.info(f"Frame {frame_idx} has label '{clean_label}' not found in references")
            skipped += 1
...
def evaluate_all_views_single_call(
    ...
    """
    logger.info(f"Evaluating all views in a single API call...")
    ...
    for ref_idx in sorted(frames_by_ref.keys()):
        ...
        parts.append(types.Part.from_bytes(
            data=reference_images_bytes[ref_idx],
            mime_type="image/png"
        ))
    ...
    for ref_idx in sorted(frames_by_ref.keys()):
        ...
        for frame in frames:
            ...
            parts.append(types.Part.from_bytes(
                data=all_frame_bytes[frame_idx],
                mime_type="image/png"
            ))

    # Add final instruction
    parts.append(types.Part.from_text(text="\n" + "="*10))
    parts.append(types.Part.from_text(text="Evaluate ALL views above and return a SINGLE verdict (is_valid: true/false)"))
    parts.append(types.Part.from_text(text="="*10))
    ...
    while True:
        try:
            ...
            if hasattr(e, "code") and e.code in (401, 403, 404):
                logger.error(f"Non-retryable error ({e.code}) in evaluate_all_views_single_call: {e}")
                raise
            sleep_time += 10
            retry_num += 1
            time.sleep(sleep_time)
            if retry_num >= 100:
                logger.error(f"Max retries (100) reached for evaluate_all_views_single_call: {e}")
                raise
            logger.error(f"Error calling generate_content: {e}, waiting: {sleep_time}s (retry {retry_num}/100)")
...
def validate_product_consistency(
    ...
    logger.info(f"Using {len(sampled_frames)} sampled frames (indices from spin validation)")

    # Step 2: Forward-fill labels if requested
    if forward_fill:
        filled_labels, filled_indices = forward_fill_labels(
            frame_labels=frame_classifications,
    ...
    if not matched_pairs:
        ...
        return True, "No frames to validate (no matches found)", {"is_valid": True, "explanation": "No frames to validate"}
    ...
    if not frames_to_evaluate:
        ...
        return True, "No frames to evaluate (SSIM selection returned empty)", {"is_valid": True, "explanation": "No frames selected"}

    # Step 5: Evaluate ALL views in a SINGLE API call
    logger.info(f"Evaluating {len(frames_to_evaluate)} frames across {len(reference_images_bytes)} references...")
