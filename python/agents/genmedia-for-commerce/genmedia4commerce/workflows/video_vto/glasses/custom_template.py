...
class StructuredPrompt(BaseModel):
    subject: str = Field(description="The description of the model appearance. Be specific on gender, optional on ethnicity and include details about the face traits, the hair and the model clothes. Use the following template: A beautiful [gender] model is wearing the same [acccurate glasses description up to 10 words] [eyeglasses|sunglasses] as in the picture. [She|He] [Appearance and clothing description]. If the user specify any characteristics of the model, for instance appearance or clothes, these have the preference with respect to what you see in the image; in this case enhance the user input. Return up to three sentences.")
    action: str = Field(description="The action description of what the model is doing. Use your expertise in commercials to comeup with amazing expressions and actions or poses.")
    scene: str = Field(description="The scene where we are shooting the commercial. If nothing is specified return a simple: 'Clean, minimalist studio environment with a uniform, soft grey background.'")
    camera_angles_and_movements: str = Field(description="Camera angles and movements specifics. If nothing is specified return a simple: 'A static close-up shot, framed from the chest up.")
    eyeglasses: Optional[str] = Field(description="A one sentence description of the main features of these eyeglasses, including material, frame color and shapes, and temple's color and shapes. Return None if the images are about sunglasses instead of eyeglasses.")
    sunglasses: Optional[str] = Field(description="A one sentence description of the main features of these sunglasses, including material, frame color and shapes, and temple's color and shapes. Return None if the images are about eyeglasses instead of sunglasses.")
    lighting: str = Field(description="The lighting settings. If nothing is specified in the text prompt, always return: 'Uniform lighting and no reflection on the (eyeglasses|sunglasses) lenses.'. Use eyeglasses or sunglasses based on the classification of the images.")
    custom_field: Optional[str] = Field(description="A custom field that we want to generate using the existing contexual information of the field itself and the prompt draft text. If the field contains instructions from the user on how to generate the field itself, follow them; if it contains a draft from the user improve it. Return None if no custom field is provided in input.")
...
The following images are useful to further customize the prompt and to understand if we have sunglasses or eyeglasses in input. Here are the images:\n"""
    ...
    images = [img for img in [model_image_bytes, product_image_bytes] if img is not None]
    content_pieces = [
        user_prompt,
        *images,
        f"Here is my draft:\n{text_prompt}",
...
Original prompt: {text_prompt}"""
    ...
    if model_image_bytes:
        ...
        content_pieces.append("Use this model image for additional context about the glasses and model appearance.")
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to generate animation prompt: {e}")