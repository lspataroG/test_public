...
def _generate_veo_batch(client, collage_img_bytes, veo_prompt, number_of_videos, duration_seconds, batch_id):
    """
...
def generate_veo(client, collage_img, veo_prompt, total_videos=4, duration_seconds=8):
    ...
    logger.info(f"Generating {total_videos} videos in {len(batch_sizes)} batch(es): {batch_sizes}")
    logger.info(f"Prompt: {veo_prompt}")

    if len(batch_sizes) == 1:
        # Single batch - no need for ThreadPoolExecutor
        return _generate_veo_batch(client, collage_img, veo_prompt, batch_sizes[0], duration_seconds, 1)
...
def create_collage(
    ...
    """
    # --- 1. Validate Inputs ---
    if model_image_bytes is None and glasses_image_bytes is None and model_side_image_bytes is None:
        return None
    ...
    if len(images) == 1:
        ...
        collage.paste(resized2, (start_x + final_w1 + image_spacing, start_y), resized2.convert('RGBA'))
        ...
        resized_model_front = model_front_img.resize((model_front_width, individual_model_height), Image.Resampling.LANCZOS)
        resized_model_side = model_side_img.resize((model_side_width, individual_model_height), Image.Resampling.LANCZOS)
        resized_glasses = glasses_img.resize((glasses_width, glasses_height), Image.Resampling.LANCZOS)
        
        # Calculate positioning
        total_content_width = left_column_width + image_spacing + glasses_width
        total_content_height = max(2 * individual_model_height + model_vertical_spacing, glasses_height)
        ...
        collage.paste(resized_model_front, (model_front_x, start_y), resized_model_front.convert('RGBA'))
        collage.paste(resized_model_side, (model_side_x, start_y + individual_model_height + model_vertical_spacing), resized_model_side.convert('RGBA'))
        ...
        collage.paste(resized_glasses, (glasses_x, glasses_y), resized_glasses.convert('RGBA'))
...
def process_veo_video_model_on_fit(video_bytes, bgcolor, fps=24, fps_to_analyze=2, start_secs_to_skip=1):
    """
    ...
    frame_list_to_check_bg = [frame for idx, frame in enumerate(frame_list[:frames_to_analyze]) if idx % skip_frequency == 0]
    frame_list_to_check_faces = [frame for idx, frame in enumerate(frame_list[frames_to_skip:frames_to_analyze]) if idx % skip_frequency == 0]
