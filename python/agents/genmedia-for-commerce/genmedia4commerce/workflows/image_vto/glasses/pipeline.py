...
class VTOResult:
    ...
    index: int = -1
    status: Literal["reference_face", "ready", "discarded", "failed", "complete"] = "ready"
    ...
    def to_sse_dict(self) -> dict:
        ...
        if self.image is not None:
            d["image_base64"] = (
                self.image_base64
                or base64.b64encode(self.image).decode("utf-8")
            )
...
async def run_glasses_vto(
    ...
    logger.info(
        "[Glasses VTO] Starting parallel preprocessing (face + glasses)"
    )
    ...
    async def _generate_variation(idx: int):
        ...
        try:
            ...
            face_weight = 0.7
            final_score = face_similarity * face_weight + glasses_score * (1 - face_weight)
