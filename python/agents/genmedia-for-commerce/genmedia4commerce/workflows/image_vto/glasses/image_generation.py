...
def evaluate_vto_image(vto_image_bytes, reference_face_bytes):
    ...
    try:
        future = submit_evaluation(reference_face_bytes, vto_image_bytes, mask_eyes=True)
...
def enhance_photo_nano(client, image_bytes, view_type="front"):
    ...
    return generate_nano(client, [prompt, image_bytes], model="gemini-3.1-flash-image-preview")
