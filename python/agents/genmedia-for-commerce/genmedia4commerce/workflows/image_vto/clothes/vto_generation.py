...
def preprocess_face_image(client, img_bytes):
    ...
    try:
        ...
        logger.warning(f"[VTO Face Preprocessing] Upscale failed, using cropped face: {e}")
    ...
    try:
        preprocessed_face = replace_background(
            client, reference_face, 0.01, VTO_BACKGROUND_COLOR,
            mask_margin_pixels=2, feather_radius=3
        )
    except Exception as e:
        logger.warning(f"[VTO Face Preprocessing] Background removal failed, using reference face: {e}")
...
def preprocess_model_image(client, img_bytes):
    ...
    try:
        ...
        img_no_bg = replace_background(
            client, img_bytes, background_color=VTO_BACKGROUND_COLOR,
            mask_margin_pixels=3, feather_radius=5
        )
...
def generate_vto(client, scenario, garment_images, preprocessed_person_images, framing="full_body", garment_descriptions=None, image_size="1K"):
    """
