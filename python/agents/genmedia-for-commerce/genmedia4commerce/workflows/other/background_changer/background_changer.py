...
# Project imports
from workflows.shared.image_utils import crop_face, replace_background, upscale_image_bytes
...
def preprocess_face_image(client, upscale_client, img_bytes):
    ...
    try:
        ...
        save_debug_image(reference_face, "03_upscaled_reference", prefix="bg_preproc_face")
...
def preprocess_person_image(client, upscale_client, img_bytes):
    ...
    try:
        ...
        img_no_bg = replace_background(client, img_bytes, contour_tolerance=0.01, background_color=None)

        logger.info("[Background Changer] Upscaling person image...")
        img_upscaled = upscale_image_bytes(upscale_client, img_no_bg, upscale_factor="x4")
...
def generate_background_change(client, preprocessed_person_image, reference_face=None, background_description=None, background_image=None):
    """
...
Generate the image.""",
    ...
    if reference_face is None:
        logger.info("[Background Changer] No reference face provided, skipping face correction")
...
def generate_background_change_only(client, preprocessed_person_image, reference_face=None, background_description=None, background_image=None):
    """
    ...
    """
    return generate_background_change(client, preprocessed_person_image, reference_face, background_description, background_image)


def evaluate_background_change_image(_client, generated_image_bytes, reference_face_bytes, model_name="ArcFace"):
    """
