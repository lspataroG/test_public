...
async def extract_uploaded_images(
    ...
    if images_b64:
        ...
        index_note = f"[System: {len(images_b64)} images uploaded, indexed as: " + \
...
async def inject_uploaded_images(
    ...
    def _resolve_index(val: str) -> str | None:
        try:
            idx = int(val)
            if 0 <= idx < len(uploaded): return uploaded[idx]
    ...
    async def _resolve_artifact(val: str) -> str | None:
        ...
        try:
            ...
            if part and part.inline_data and part.inline_data.data:
                ...
                if isinstance(data, bytes): return base64.b64encode(data).decode("utf-8")
                return data
    ...
    def _resolve_url(val: str) -> str | None:
        ...
        for prefix in ["https://storage.cloud.google.com/", "https://storage.googleapis.com/"]:
            if val.startswith(prefix):
                return "gs://" + val[len(prefix):].split("?")[0]
        if not val.startswith(("http://", "https://")): return None
        try:
            import httpx
    ...
    for param, val in list(args.items()):
        if not param.endswith("_base64"): continue
        if isinstance(val, str) and len(val) < 1000:
            res = await _resolve(val)
            if res: args[param] = res
        elif isinstance(val, list):
            args[param] = [((await _resolve(i) if isinstance(i, str) and len(i) < 1000 else i)) for i in val]
    return None
...
def _strip_images_from_result(result: dict, tool_name: str) -> tuple[dict, dict[str, bytes]]:
    artifacts: dict[str, bytes] = {}
    def _ext(k): return ".mp4" if "video" in k else ".png"
    ...
    def _clean_dict(d: dict, prefix: str) -> dict:
        ...
        for k, v in d.items():
            ...
            elif isinstance(v, dict): clean[k] = _clean_dict(v, f"{prefix}{k}_")
            elif isinstance(v, list):
                clean[k] = [(_clean_dict(i, f"{prefix}{k}_{idx}_") if isinstance(i, dict) else i) for idx, i in enumerate(v)]
            else: clean[k] = v
        return clean
...
def _generate_signed_url(url: str) -> str | None:
    try:
        if not (url.startswith("gs://") or "storage.cloud.google.com" in url or "storage.googleapis.com" in url):
            return None
        client = storage.Client()
        path = url.replace("gs://", "").replace("https://storage.cloud.google.com/", "").replace("https://storage.googleapis.com/", "").split("?")[0]
        parts = path.split("/", 1)
...
async def _fetch_image_data(url: str) -> tuple[bytes, str] | tuple[None, None]:
    try:
        ...
        if url.startswith("gs://") or "storage.cloud.google.com" in url or "storage.googleapis.com" in url:
            path = url.replace("gs://", "").replace("https://storage.cloud.google.com/", "").replace("https://storage.googleapis.com/", "").split("?")[0]
            parts = path.split("/", 1)
            blob = client.bucket(parts[0]).blob(parts[1])
...
async def process_text_for_gcs_urls(text: str, runner_or_context, user_id="test_user", session_id="test_session") -> str:
    ...
    for url in list(set(urls))[:15]:
        ...
        if not final_url:
            ...
            if data:
                ...
                if hasattr(runner_or_context, "save_artifact"):
                     await runner_or_context.save_artifact(fname, types.Part(inline_data=types.Blob(mime_type=mime, data=data)))
        ...
        if final_url: text = text.replace(url, final_url)
    return text
...
async def handle_tool_response(
    ...
    if tool_response is None: return None
    ...
    if isinstance(tool_response, dict):
        ...
        try:
            ...
            for name, data in artifacts.items():
                ...
                await tool_context.save_artifact(name, types.Part(inline_data=types.Blob(mime_type=mime, data=data)))
            if artifacts:
...
""",
    tools=[
        McpToolset(connection_params=SseConnectionParams(url=MCP_SERVER_URL, timeout=30, sse_read_timeout=600)),
        google_search,
    ],
