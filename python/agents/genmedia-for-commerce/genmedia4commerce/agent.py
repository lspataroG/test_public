...
async def _dump_all_artifacts(callback_context: CallbackContext, global_session_id: str):
    ...
    try:
        ...
        print(f"[Debug] All artifact keys ({len(keys)}): {keys}")
        ...
        for i, key in enumerate(keys):
            ...
            if artifact and artifact.inline_data and artifact.inline_data.data:
                ...
                print(f"[Debug] Saved artifact {i}: {key} → gs://{MEDIA_BUCKET}/{blob_path}")
            else:
                print(f"[Debug] Artifact {i}: {key} — no inline_data")
    except Exception as e:
...
async def build_conversational_history(
    ...
    print(f"[before_model] GE artifact keys from message markers: {ge_artifact_keys}")
    ...
    # ADK Case: inline_data in the last user message
        ...
        print(f"[before_model] Last user message has {len(parts)} parts")
...
async def inject_uploaded_images(
    ...
    for param, val in list(args.items()):
        ...
        if isinstance(val, str) and len(val) < 500:
            ...
            print(f"[before_tool] Resolving {param}: {val} → {gcs_uri}")
            ...
            print(f"[before_tool] Resolving {param}: {len(uris)} filenames → {uris}")
            downloads = await asyncio.gather(
                *[loop.run_in_executor(None, _download_from_gcs, uri) for uri in uris]
            )
            for (i, _), data in zip(items_to_resolve, downloads):
                if data:
...
async def handle_tool_response(
    ...
    print(f"[after_tool] ENTER tool={tool_name}, result_type={type(result).__name__}")
    ...
    if raw_text is None:
        print(f"[after_tool] EARLY EXIT: raw_text is None for {tool_name}")
        return None

    try:
        parsed = json.loads(raw_text)
    except (json.JSONDecodeError, TypeError):
        print(f"[after_tool] EARLY EXIT: JSON parse failed for {tool_name}, raw_text[:200]={raw_text[:200]}")
        return None

    global_session_id = tool_context.state.get("global_session_id")
    timestamp = str(int(time.time()))
    print(f"[after_tool] tool={tool_name}, global_session_id={global_session_id}")
    print(f"[after_tool] parsed type={type(parsed).__name__}, keys={list(parsed.keys()) if isinstance(parsed, dict) else 'N/A'}")

    fr_parts = []

    if tool_name == "catalog_search":
        items = parsed if isinstance(parsed, list) else []
        print(f"[after_tool] catalog_search: {len(items)} items")
        # Download catalog images in parallel for artifact saving
        gcs_uris = [item.get("img_path", "") for item in items if item.get("img_path")]
        print(f"[after_tool] catalog_search: {len(gcs_uris)} gcs_uris to download")
        downloaded = await resolve_media(gcs_uris) if gcs_uris else []
        print(f"[after_tool] catalog_search: {len(downloaded)} downloaded")
        ...
        for i, item in enumerate(items):
            ...
            mime = get_mime_type_from_bytes(downloaded[i])
            part = types.Part(
                inline_data=types.Blob(mime_type=mime, data=downloaded[i])
            )
            artifact_tasks.append(
                tool_context.save_artifact(filename=filename, artifact=part)
            )
        print(f"[after_tool] catalog_search: {len(copy_tasks)} copy_tasks, {len(artifact_tasks)} artifact_tasks")
        await asyncio.gather(*copy_tasks, *artifact_tasks)
        print(f"[after_tool] catalog_search: done saving artifacts")
        tool_result = {"output": "\n\n".join(item_lines)}
    else:
        # Extract media (base64 → bytes, gs:// URIs → str) and resolve
        media = extract_media(parsed, tool_name)
        print(f"[after_tool] {tool_name}: extracted {len(media)} media items (types: {[type(m).__name__ for m in media]})")
        resolved = await resolve_media(media) if media else []
        print(f"[after_tool] {tool_name}: resolved {len(resolved)} media items")
        ...
        print(f"[after_tool] {tool_name}: {len(upload_tasks)} upload_tasks, {len(artifact_tasks)} artifact_tasks")
        await asyncio.gather(*upload_tasks, *artifact_tasks)
        print(f"[after_tool] {tool_name}: done saving artifacts, filenames={filenames}")
        tool_result = {
