...
CLASSES = ["front", "front_right", "front_left", "top_front", "right", "left",
           "back", "back_right", "back_left", "sole", "invalid", "multiple"]
CLASS_TO_IDX = {c: i for i, c in enumerate(CLASSES)}
SIDE_CLASSES = {"front", "front_right", "front_left", "top_front", "right", "left",
                "back", "back_right", "back_left", "sole"}
...
def compute_metrics(y_true, y_pred):
    ...
    # Side-to-side errors
    s2s = sum(1 for i in range(len(y_true)) if y_true[i] != y_pred[i]
              and y_true[i] in SIDE_CLASSES and y_pred[i] in SIDE_CLASSES)

    # Invalid F1
    inv_f1 = f1_score(y_true, y_pred, labels=["invalid"], average=None, zero_division=0)[0]

    # Side misclassified as invalid and vice versa
    side_as_inv = sum(1 for i in range(len(y_true)) if y_true[i] in SIDE_CLASSES and y_pred[i] == "invalid")
    inv_as_side = sum(1 for i in range(len(y_true)) if y_true[i] == "invalid" and y_pred[i] in SIDE_CLASSES)
...
def main():
    ...
    sys.path.insert(0, os.path.join(os.path.dirname(__file__), "..", "..", "genmedia4commerce"))
    from workflows.spinning.r2v.shoes.classify_shoes import SHOE_CLASSIFICATION_SYSTEM_PROMPT
    ...
    config = types.GenerateContentConfig(
        temperature=0,
        system_instruction=[types.Part.from_text(text=SHOE_CLASSIFICATION_SYSTEM_PROMPT)],
    ...
    for ep in endpoints:
        print(f"  {ep.display_name} ({ep.resource_name.split('/')[-1]}) updated={ep.update_time}")
    ...
    for ep in endpoints:
        ...
        predict_fn = partial(predict_shoe_position, endpoint_name, client=client, config=config)
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            futures = {executor.submit(predict_fn, img): i for i, img in enumerate(images)}
            done = 0
        ...
        elapsed = time.time() - t0
        print(f"  Inference done: {len(images)} samples in {elapsed:.0f}s ({len(images)/elapsed:.1f} samples/s)")
    ...
    print(f"{'Rank':<5} {'Display Name':<55} {'Acc':>7} {'F1mac':>7} {'Errors':>7} "
          f"{'S2S':>5} {'InvF1':>7} {'S->I':>5} {'I->S':>5}")
    print("-" * 110)
    for i, r in enumerate(all_results):
        marker = " <-- BEST" if i == 0 else ""
        print(f"{i+1:<5} {r['display_name']:<55} {r['acc']:>7.4f} {r['f1_macro']:>7.4f} "
