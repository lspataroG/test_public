...
_DIM = 4
_NUM = 5
_FAKE_EMBEDDINGS = np.random.default_rng(42).standard_normal((_NUM, _DIM)).astype(
    np.float32
)
...
class TestSearchForOutfitItem:
    ...
    def test_search_error_populates_error_field(self, _patch_vector_search_load):
        ...
        with patch.object(
            vs, "search", side_effect=RuntimeError("embedding failed")
        ):
            result = vs.search_for_outfit_item(item, k=5)
...
class TestSearch:
    ...
    def test_calls_embed_and_search_by_vector(self, _patch_vector_search_load):
        ...
        with (
            patch.object(
                vs, "embed_query", return_value=fake_embedding
            ) as mock_embed,
            patch.object(
