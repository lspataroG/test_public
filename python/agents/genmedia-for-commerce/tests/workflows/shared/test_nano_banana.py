...
from workflows.shared.nano_banana import generate_nano, NanoTimeoutError, NANO_TIMEOUT_SECONDS
...
class TestGenerateNano:
    ...
    def test_generates_image_from_text_and_image(self, mock_gemini_client, sample_image_bytes, generated_image_bytes):
    ...
    def test_uses_default_config_when_none(self, mock_gemini_client, generated_image_bytes):
        """Should use default config with IMAGE modality when none provided."""
        with patch('workflows.shared.nano_banana.get_generate_content_config') as mock_get_config:
            mock_config = Mock()
            ...
            mock_get_config.assert_called_once_with(response_modalities=["IMAGE", "TEXT"])
    ...
    def test_handles_multiple_images(self, mock_gemini_client, sample_image_bytes, generated_image_bytes):
    ...
    def test_filters_image_parts_from_response(self, mock_gemini_client, generated_image_bytes):
...
class TestGenerateNanoContentsFormat:
    ...
    def test_processes_all_input_pieces(self, mock_gemini_client, sample_image_bytes, generated_image_bytes):
