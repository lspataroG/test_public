...
class TestGetInterpolationPrompt:
    ...
    def test_returns_string(self, red_image_bytes):
        ...
        mock_client = Mock()
        with patch('workflows.spinning.interpolation.other.interpolation_utils.generate_generic_product_title', return_value="a product"):
            prompt = get_interpolation_prompt(mock_client, all_images_bytes=[red_image_bytes])
...
class TestPostProcessSingleVideo:
    ...
    def test_is_first_video_keeps_first_frame(self, sample_video_bytes, blue_image_bytes):
    ...
    def test_not_first_video_removes_first_frame(self, sample_video_bytes, blue_image_bytes):
