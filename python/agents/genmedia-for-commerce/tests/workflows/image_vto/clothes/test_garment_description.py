...
class TestDescribeGarmentForVto:
    ...
    def test_parses_valid_json_response(self, mock_generate, mock_client, garment_image_bytes):
    ...
    @patch("workflows.image_vto.clothes.garment_description.generate_gemini")
    def test_handles_missing_keys_in_response(self, mock_generate, mock_client, garment_image_bytes):
    ...
    @patch("workflows.image_vto.clothes.garment_description.generate_gemini")
    def test_returns_fallback_on_json_parse_error(self, mock_generate, mock_client, garment_image_bytes):
    ...
    @patch("workflows.image_vto.clothes.garment_description.generate_gemini")
    def test_returns_fallback_on_api_exception(self, mock_generate, mock_client, garment_image_bytes):
    ...
    @patch("workflows.image_vto.clothes.garment_description.generate_gemini")
    def test_passes_image_and_client_to_gemini(self, mock_generate, mock_client, garment_image_bytes):
        """Should pass the garment image and client to generate_gemini."""
        mock_generate.return_value = json.dumps({"general": "", "details": ""})

        describe_garment_for_vto(mock_client, garment_image_bytes, model="gemini-custom")
...
class TestDescribeAllGarments:
    ...
    @patch("workflows.image_vto.clothes.garment_description.describe_garment_for_vto")
    def test_describes_garments_in_parallel(self, mock_describe, mock_client, garment_image_bytes):
