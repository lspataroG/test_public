...
class TestBuildDescriptionBlock:
    ...
    def test_single_garment_uses_garment_label(self):
        ...
        result = _build_description_block([{"general": "Nike running shorts", "details": ""}])
...
class TestPreprocessFaceImage:
    ...
    def test_returns_none_tuple_when_no_face_detected(
        self, mock_crop, mock_upscale, mock_replace_bg, mock_save, mock_client, img_bytes
    ):
...
class TestGenerateVto:
    ...
    def test_returns_both_steps_on_success(self, mock_nano, mock_save, mock_client, img_bytes):
    ...
    @patch("workflows.image_vto.clothes.vto_generation.save_debug_image")
    @patch("workflows.image_vto.clothes.vto_generation.generate_nano")
    def test_returns_none_when_step1_fails(self, mock_nano, mock_save, mock_client, img_bytes):
    ...
    @patch("workflows.image_vto.clothes.vto_generation.save_debug_image")
    @patch("workflows.image_vto.clothes.vto_generation.generate_nano")
    def test_returns_step2_none_when_step2_fails(self, mock_nano, mock_save, mock_client, img_bytes):
