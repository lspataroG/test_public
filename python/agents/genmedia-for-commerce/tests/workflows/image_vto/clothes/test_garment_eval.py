...
class TestEvaluateGarments:
    ...
    def test_normalizes_scores_to_100_scale(self, mock_eval, mock_client, generated_image_bytes, garment_image_bytes):
    ...
    @patch("workflows.image_vto.clothes.garment_eval.evaluate_garment")
    def test_averages_multiple_garment_scores(self, mock_eval, mock_client, generated_image_bytes, garment_image_bytes):
    ...
    @patch("workflows.image_vto.clothes.garment_eval.evaluate_garment")
    def test_discard_logic_uses_threshold_of_5(self, mock_eval, mock_client, generated_image_bytes, garment_image_bytes):
        ...
        result = evaluate_garments(
            mock_client, generated_image_bytes, [garment_image_bytes, garment_image_bytes],
        )
    ...
    @patch("workflows.image_vto.clothes.garment_eval.evaluate_garment")
    def test_returns_garment_details_list(self, mock_eval, mock_client, generated_image_bytes, garment_image_bytes):
    ...
    @patch("workflows.image_vto.clothes.garment_eval.evaluate_garment")
    def test_uses_empty_descriptions_when_none_provided(self, mock_eval, mock_client, generated_image_bytes, garment_image_bytes):
    ...
    @patch("workflows.image_vto.clothes.garment_eval.evaluate_garment")
    def test_forwards_descriptions_to_evaluate_garment(self, mock_eval, mock_client, generated_image_bytes, garment_image_bytes):
        """Should pass garment description fields to evaluate_garment."""
        mock_eval.return_value = {"score": 8, "explanation": "Good"}

        descriptions = [{"general": "Blue shirt", "details": "[EXTERIOR] Logo on chest"}]
...
class TestEvaluateWearingQuality:
    ...
    def test_returns_valid_score_and_explanation(self, mock_generate, mock_client, generated_image_bytes):
    ...
    @patch("workflows.image_vto.clothes.garment_eval.generate_gemini")
    def test_clamps_score_to_valid_range(self, mock_generate, mock_client, generated_image_bytes):
        """Should clamp scores to the 0-3 range."""
        # Above upper bound
        mock_generate.return_value = json.dumps({"explanation": "Perfect", "score": 10})
        assert evaluate_wearing_quality(mock_client, generated_image_bytes)["score"] == 3

        # Below lower bound
        mock_generate.return_value = json.dumps({"explanation": "Broken", "score": -5})
        assert evaluate_wearing_quality(mock_client, generated_image_bytes)["score"] == 0

    @patch("workflows.image_vto.clothes.garment_eval.generate_gemini")
    def test_returns_error_result_on_failure(self, mock_generate, mock_client, generated_image_bytes):
