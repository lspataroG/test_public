...
class TestPreprocessPersonImage:
    ...
    @patch("workflows.other.background_changer.background_changer.upscale_image_bytes")
    @patch("workflows.other.background_changer.background_changer.replace_background")
    def test_removes_bg_then_upscales(self, mock_replace_bg, mock_upscale, person_image_bytes):
        ...
        result = preprocess_person_image(mock_client, mock_upscale_client, person_image_bytes)

        assert result == b"upscaled"
        mock_replace_bg.assert_called_once()
        # Upscale should receive the bg-removed output
        mock_upscale.assert_called_once_with(mock_upscale_client, b"no_bg", upscale_factor="x4")

    @patch("workflows.other.background_changer.background_changer.upscale_image_bytes")
    @patch("workflows.other.background_changer.background_changer.replace_background")
    def test_returns_original_on_error(self, mock_replace_bg, mock_upscale, person_image_bytes):
...
class TestGenerateBackgroundChange:
    ...
    @patch("workflows.other.background_changer.background_changer.generate_nano")
    def test_image_background_no_face(self, mock_nano, person_image_bytes, background_image_bytes):
    ...
    @patch("workflows.other.background_changer.background_changer.generate_nano")
    def test_with_reference_face_runs_two_steps(self, mock_nano, person_image_bytes, face_image_bytes):
        ...
        assert result == {"step1_image": b"step1_result", "step2_image": b"step2_result"}
    ...
    @patch("workflows.other.background_changer.background_changer.generate_nano")
    def test_step2_failure_returns_step1_only(self, mock_nano, person_image_bytes, face_image_bytes):
...
class TestEvaluateBackgroundChangeImage:
    ...
    def test_returns_evaluation_result(self, mock_submit, face_image_bytes, person_image_bytes):
        ...
        result = evaluate_background_change_image(Mock(), person_image_bytes, face_image_bytes)

        assert result["similarity_percentage"] == 85.0
        assert result["face_detected"] is True

    @patch("workflows.other.background_changer.background_changer.submit_evaluation")
    def test_returns_fallback_on_exception(self, mock_submit, face_image_bytes, person_image_bytes):
